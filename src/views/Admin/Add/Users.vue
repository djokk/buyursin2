  ...
  <section class="users">
      ...
      <div class="card-custom">
        <div class="table-custom">
          <div class="table-custom__header">
            <p class="w-1">{{ $t('username') }}</p>
            <p class="w-2">{{ $t('branchName') }}</p>
            <p class="w-3">{{ $t('userType') }}</p>
            <p class="w-4"></p>
          </div>
          <div class="table-custom__body">
            <div v-for="item in info" :key="item.id" class="table-custom__body-item">
              <p class="w-1">{{ item.username }}</p>
              <p class="w-2">{{ item.branchName == null ? '' : item.branchName }}</p>
              <p class="w-3">{{ item.is_superuser == true ? 'ADMIN' : 'EMPLOYEE' }}</p>
              <div class="w-4 btns">
    ...
    <div v-if="modalAdd" class="modal-add">
      <form @submit.prevent="addUser()" class="form" action="">
        <div class="selects">
          <multiselect v-model="branchTypeValue" :class="$v.branchTypeValue.$error ? 'is-invalid': ''" placeholder="" label="name" track-by="id" :options="branchTypeOptions" :multiple="false" :taggable="false" :searchable="false"></multiselect>
          <span>{{ $t('groupMatter') }}</span>
        </div>
        <div class="name">
          <input v-model="userName" :class="$v.userName.$error ? 'is-invalid': ''" class="input" type="text">
        ...
        <div class="checkbox">
          <input v-model="superUser" class="input" type="checkbox">
...
export default {
  ...
  methods: {
    getBranches() {
      ...
      axios.get(`${this.api}/branches/`, {
        ...
        .then(response => {
          ...
          this.branchTypeOptions = response.data;
        })
        .catch(error => {
    ...
    addUser() {
      this.$v.$touch()
      if (this.$v.branchTypeValue.$invalid || this.$v.password.$invalid || this.$v.userName.$invalid) {
        this.$toast.open({
        ...
        this.loading = true;
        axios.post(`${this.api}/createUser/`, {
            "branchId": this.branchTypeValue.id,
    ...
    changeModalAdd(item) {
      ...
      this.superUser = item.is_superuser;
      this.branchTypeValue = {
        'id': item.branchId,
