<template>
  <section class="worktemplates">
        ...
        <div class="table-custom">
          ...
          <div class="table-custom__body">
            <div v-for="(item,index) in info" :key="item.id" class="table-custom__body-item">
              ...
              <p class="w-3">{{ item.pricePerOne }} sum</p>
              <div class="w-4 btns">
    ...
    <div v-if="modalDelete" class="modal-delete">
      <form @submit.prevent="delWorktemplates()" class="form" action="">
        <p>{{ $t('textDelete') }}</p>
        <div class="btns">
    ...
    <div v-if="modalAdd" class="modal-add">
      <form class="form" action="">
        <div class="name">
          <input v-model="nameWorktemplates" :class="$v.nameWorktemplates.$error ? 'is-invalid': ''" class="input" type="text">
          <span>{{ $t('name') }}</span>
        </div>
        <div class="name">
          <input v-model="priceWorktemplates" :class="$v.priceWorktemplates.$error ? 'is-invalid': ''" class="input" type="number">
          <span>{{ $t('price') }}</span>
        </div>
        <div class="btns">
          <button v-if="btnChange != true" @click.prevent="addWorktemplates()" class="btn btn-success" type="button">{{ $t('add') }}</button>
          <button v-else-if="btnChange == true" @click.prevent="editWorktemplates()" class="btn btn-warning" type="button">{{ $t('change') }}</button>
...
export default {
  ...
  methods: {
    loadWorktemplates() {
      this.loading = true;
      axios.get(`${this.api}/worktemplates/`, {
          headers: {
    ...
    addWorktemplates() {
      ...
      if (!this.$v.$invalid) {
        this.loading = true;
        axios.post(`${this.api}/worktemplates/`, {
            name: this.nameWorktemplates,
            pricePerOne: this.priceWorktemplates
          }, 
          {
            headers: {
              'Authorization': `Token ${this.token}`
          ...
          .then(responce => {
            ...
            if(responce.data.сode == 1) {
              ...
              this.$toast.error(responce.data.msg);
            }
          })
    ...
    delWorktemplates() {
      this.loading = true;
      axios.delete(`${this.api}/worktemplate-delete/${this.idWorktemplates}/`), {
          headers: {
            'Authorization': `Token ${this.token}`
          }
        }
    ...
    clear() {
      ...
      this.btnChange = false;
      this.nameWorktemplates = '';
      this.priceWorktemplates = '';
      this.idWorktemplates = '';
      this.$v.$reset();
    },
    openModalDel(id) {
      this.idWorktemplates = id;
      this.modalDelete = true;
    },
    changeModalAdd(item) {
      this.idWorktemplates = item.id;
      this.nameWorktemplates = item.name;
      this.priceWorktemplates = item.pricePerOne;
      this.btnChange = true;
      this.modalAdd = true;
    },
  },
